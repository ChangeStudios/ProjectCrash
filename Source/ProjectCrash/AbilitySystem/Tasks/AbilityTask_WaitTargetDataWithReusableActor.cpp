...
#include "AbilityTask_WaitTargetDataWithReusableActor.h"
...
UAbilityTask_WaitTargetDataWithReusableActor* UAbilityTask_WaitTargetDataWithReusableActor::CreateWaitTargetDataWithReusableActorProxy(UGameplayAbility* OwningAbility, FName TaskInstanceName, TEnumAsByte<EGameplayTargetingConfirmation::Type> ConfirmationType, AGameplayAbilityTargetActor* InTargetActor, bool bCreateKeyIfNotValidForMorePredicting)
{
	...
	// Create a new task.
	UAbilityTask_WaitTargetDataWithReusableActor* MyTask = NewAbilityTask<UAbilityTask_WaitTargetDataWithReusableActor>(OwningAbility, TaskInstanceName);
...
void UAbilityTask_WaitTargetDataWithReusableActor::ExternalConfirm(bool bEndTask)
{
...
void UAbilityTask_WaitTargetDataWithReusableActor::ExternalCancel()
{
...
void UAbilityTask_WaitTargetDataWithReusableActor::Activate()
{
	if (!IsValid(this))
	{
		return;
...
void UAbilityTask_WaitTargetDataWithReusableActor::RegisterServerTargetDataCallbacks()
{
	...
	if (!bLocallyControlled)
	{
		...
		if (!bShouldProduceTargetDataOnServer)
		{
			...
			AbilitySystemComponent->AbilityTargetDataSetDelegate(SpecHandle, ActivationPredictionKey).AddUObject(this, &UAbilityTask_WaitTargetDataWithReusableActor::OnTargetDataReplicated);
			AbilitySystemComponent->AbilityTargetDataCancelledDelegate(SpecHandle, ActivationPredictionKey).AddUObject(this, &UAbilityTask_WaitTargetDataWithReusableActor::OnTargetDataReplicatedCancelled);
...
void UAbilityTask_WaitTargetDataWithReusableActor::ConfigureTargetActor()
{
	check(TargetActor);
	check(Ability);
	...
	TargetActor->TargetDataReadyDelegate.AddUObject(this, &UAbilityTask_WaitTargetDataWithReusableActor::OnTargetDataReady);
	TargetActor->CanceledDelegate.AddUObject(this, &UAbilityTask_WaitTargetDataWithReusableActor::OnTargetDataCancelled);
}

void UAbilityTask_WaitTargetDataWithReusableActor::SetUpTargetingConfirmation()
{
	check(TargetActor);
	check(Ability);
...
void UAbilityTask_WaitTargetDataWithReusableActor::OnDestroy(bool bInOwnerFinished)
{
...
void UAbilityTask_WaitTargetDataWithReusableActor::OnTargetDataReady(const FGameplayAbilityTargetDataHandle& Data)
{
...
void UAbilityTask_WaitTargetDataWithReusableActor::OnTargetDataCancelled(const FGameplayAbilityTargetDataHandle& Data)
{
	UAbilitySystemComponent* ASC = AbilitySystemComponent.Get();
	if (!ASC)
	{
...
void UAbilityTask_WaitTargetDataWithReusableActor::OnTargetDataReplicated(const FGameplayAbilityTargetDataHandle& Data, FGameplayTag Activation)
{ 
...
void UAbilityTask_WaitTargetDataWithReusableActor::OnTargetDataReplicatedCancelled()
{
...
bool UAbilityTask_WaitTargetDataWithReusableActor::ShouldReplicateDataToServer()
{
