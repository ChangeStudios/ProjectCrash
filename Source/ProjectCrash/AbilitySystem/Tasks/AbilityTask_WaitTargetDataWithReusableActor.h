...
#include "AbilityTask_WaitTargetDataWithReusableActor.generated.h"
 ...
 * destroys the target actor each time the task is called, which is not performant. This task does not destroy the
 * target actor, allowing target actors to be re-used and re-configured for each ability activation. This implementation
 * also adds support for multiple targets.
 *
 * The spawning and configuring of the target actor must be handled by the ability, as this task is agnostic to the
 ...
 */
UCLASS()
class PROJECTCRASH_API UAbilityTask_WaitTargetDataWithReusableActor : public UAbilityTask
{
	GENERATED_BODY()
...
public:
	...
	/**
	 * Waits to receive target data from a specified target actor.
	 *
	 * The target actor must be spawned and configured before being passed into this task. The target actor should only
	 * be spawned during the first activation and be reused for subsequent activations. Target actors can be
	 * reconfigured for each use, if necessary.
	 *
	 * Note that for instanced-per-execution abilities, the target actor cannot be cached for re-use, and must instead
	 * be created and destroyed for each activation.
	 *
	 ...
	 *													ability.
	 */
	UFUNCTION(BlueprintCallable,  Category = "Ability|Tasks", Meta = (DisplayName = "Wait Target Data With Reusable Actor",
		HidePin = "OwningAbility", DefaultToSelf = "OwningAbility", BlueprintInternalUseOnly = "true", HideSpawnParms = "Instigator"))
	static UAbilityTask_WaitTargetDataWithReusableActor* CreateWaitTargetDataWithReusableActorProxy
	(
