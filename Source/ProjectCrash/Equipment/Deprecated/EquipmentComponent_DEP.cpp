...
UEquipmentComponent_DEP::UEquipmentComponent_DEP() :
...
void UEquipmentComponent_DEP::OnRegister()
{
	Super::OnRegister();
	...
	Char->GetComponents(UEquipmentComponent_DEP::StaticClass(), EquipmentComponents);
...
void UEquipmentComponent_DEP::OnComponentDestroyed(bool bDestroyingHierarchy)
{
	if (HasAuthority())
	{
...
void UEquipmentComponent_DEP::EquipSet(UEquipmentSetDefinition* SetToEquip)
{
...
void UEquipmentComponent_DEP::TemporarilyEquipSet(UEquipmentSetDefinition* SetToTemporarilyEquip)
{
...
bool UEquipmentComponent_DEP::UnequipTemporarySet()
{
...
bool UEquipmentComponent_DEP::PlayEquipmentMontage(FGameplayTag EquipmentAnimTag)
{
...
void UEquipmentComponent_DEP::DetachEquipment()
{
...
void UEquipmentComponent_DEP::EquipSet_Internal(UEquipmentSetDefinition* SetToEquip, FEquipmentSetHandle& OutEquippedSet)
{
	check(SetToEquip);
...
void UEquipmentComponent_DEP::UnequipSet_Internal(FEquipmentSetHandle& SetToUnequip, bool bUnequipTemporarily)
{
	if (!GetOwner())
	{
		return;
...
void UEquipmentComponent_DEP::OnRep_EquippedSet(UEquipmentSetDefinition* PreviouslyEquippedSet)
{
...
void UEquipmentComponent_DEP::OnRep_TemporarilyEquippedSet(UEquipmentSetDefinition* PreviouslyTemporarilyEquippedSet)
{
	// Check on the client if we predicted this change. If we did, we can now confirm the prediction as successful.
	if (!HasAuthority() && GetOwner()->HasLocalNetOwner())
	{
...
void UEquipmentComponent_DEP::Client_EquipPredictionFailed_Implementation(bool bTemporarySet)
{
	if (HasAuthority())
	{
		return;
...
void UEquipmentComponent_DEP::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const
{
	Super::GetLifetimeReplicatedProps(OutLifetimeProps);

	DOREPLIFETIME_CONDITION_NOTIFY(UEquipmentComponent_DEP, EquippedSet, COND_None, REPNOTIFY_Always);
	DOREPLIFETIME_CONDITION_NOTIFY(UEquipmentComponent_DEP, TemporarilyEquippedSet, COND_None, REPNOTIFY_Always);
}
