...
bool UCrashCollisionLibrary::ConeTraceMulti(
	...
	float DrawTime)
{
	...
	static const FName ConeTraceMultiName(TEXT("ConeTraceMulti"));
	FCollisionQueryParams Params = ConfigureCollisionParams(ConeTraceMultiName, bTraceComplex, ActorsToIgnore, bIgnoreSelf, WorldContextObject);
...
FCollisionQueryParams UCrashCollisionLibrary::ConfigureCollisionParams(FName TraceTag, bool bTraceComplex, const TArray<AActor*>& ActorsToIgnore, bool bIgnoreSelf, const UObject* WorldContextObject)
{
	FCollisionQueryParams Params(TraceTag, SCENE_QUERY_STAT_ONLY(CrashCollisionLibrary), bTraceComplex);
	Params.bReturnPhysicalMaterial = true;
	Params.bReturnFaceIndex = !UPhysicsSettings::Get()->bSuppressFaceRemapTable; // Ask for face index, as long as it isn't globally disabled.
	Params.AddIgnoredActors(ActorsToIgnore);
	if (bIgnoreSelf)
	{
		...
		{
			// If the context object isn't an actor, assume the first actor in its owner chain is "self."
